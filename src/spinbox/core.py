...
class HilbertOperator:
    ...
    def apply_onebody_operator(
        self,
        particle_index: int,
        spin_matrix: np.ndarray,
        isospin_matrix: np.ndarray = None,
    ):
        ...
        obo = [
            np.identity(self.n_basis, dtype=complex) for _ in range(self.n_particles)
        ]
        if self.isospin:
    ...
    def apply_sigma(self, particle_index: int, dimension: int) -> "HilbertOperator":
        ...
        :rtype: HilbertOperator
        """
        return self.apply_onebody_operator(
...
class ProductState:
    ...
    def __init__(self, n_particles: int, coefficients=None, ketwise=True, isospin=True):
        ...
        if coefficients is None:
            ...
            assert type(coefficients) == np.ndarray
            ket_condition = (
                coefficients.shape == (n_particles, self.n_basis, 1)
            ) and ketwise
            bra_condition = (
                coefficients.shape == (n_particles, 1, self.n_basis)
            ) and not ketwise
            if not ket_condition and not bra_condition:
                raise ValueError(
                    "Inconsistent initialization of state vector. \n\
    ...
    def inner(self, other: "ProductState") -> complex:
        ...
        return np.prod(
            [
                np.dot(self.coefficients[i], other.coefficients[i])
    ...
    def outer(self, other: "ProductState") -> "ProductState":
        ...
        for i in range(self.n_particles):
            out.coefficients[i] = np.matmul(
                self.coefficients[i], other.coefficients[i], dtype=complex
            )
        return out
    ...
    def generate_basis_states(self) -> list["ProductState"]:
        ...
        """
        coeffs_list = [
            np.concatenate(x).reshape(self.n_particles, self.n_basis, 1)
            for x in itertools.product(
                list(np.eye(self.n_basis)), repeat=self.n_particles
            )
        ]
        out = [
            ProductState(
...
class ProductOperator:
    ...
    def __init__(self, n_particles: int, isospin=True):
        ...
        self.coefficients = np.stack(
            self.n_particles * [np.identity(self.n_basis)], dtype=complex
        )
    ...
    def multiply_state(self, other: ProductState) -> ProductState:
        ...
        for i in range(self.n_particles):
            out.coefficients[i] = np.matmul(
                self.coefficients[i], out.coefficients[i], dtype=complex
            )
        return out
    ...
    def to_full_basis(self) -> "HilbertOperator":
        ...
        new_coeffs = new_coeffs.reshape(
            self.n_basis**self.n_particles, self.n_basis**self.n_particles
        )
        return HilbertOperator(
            n_particles=self.n_particles, coefficients=new_coeffs, isospin=self.isospin
        )
    ...
    def apply_onebody_operator(
        ...
        out = self.copy()
        out.coefficients[particle_index] = np.matmul(
            onebody_matrix, out.coefficients[particle_index], dtype=complex
        )
        return out
                    ...
                    for a in range(3):
                        for b in range(3):
                            for c in range(3):
                                ...
                                self.coefficients[a, i, b, j, c, k] = self.coefficients[
                                    a, i, b, k, c, j
                                ]
                                self.coefficients[a, i, b, j, c, k] = self.coefficients[
                                    a, j, b, i, c, k
...
class NuclearPotential:
    ...
    def random(self, seed):
        ...
        self.sigma.random(mean=-5.0, scale=5.0, seed=next(seeder))
        self.sigmatau.random(mean=-5.0, scale=5.0, seed=next(seeder))
        self.tau.random(mean=-5.0, scale=5.0, seed=next(seeder))
        self.coulomb.random(mean=1.0, scale=1.0, seed=next(seeder))
        self.spinorbit.random(mean=-0.1, scale=0.1, seed=next(seeder))
        self.sigma_3b.random(mean=-2.0, scale=2.0, seed=next(seeder))
        return self
...
class ExactPropagator(Propagator):
    ...
    def __init__(self, n_particles, dt: float, isospin=True, include_prefactors=True):
        ...
        self._sig = [
            [
                HilbertOperator(n_particles, self.isospin).apply_sigma(i, a)
                for a in [0, 1, 2, 3]
            ]
        ...
        self._tau = [
            [
                HilbertOperator(n_particles, self.isospin).apply_tau(i, a)
                for a in [0, 1, 2, 3]
            ]
            for i in range(n_particles)
        ]
    ...
    def force_sigma(
        self, coupling_array: CouplingArray, i: int, j: int
    ) -> HilbertOperator:
        ...
        for a in range(3):
            for b in range(3):
                out += (
                    self._sig[i][a]
                    .multiply_operator(self._sig[j][b])
                    .scale(coupling_array[a, i, b, j])
                )
        return out
        ...
        for a in range(3):
            for b in range(3):
                for c in range(3):
                    ...
                    op = op.multiply_operator(self._sig[i][a]).multiply_operator(
                        self._tau[i][c]
                    )
                    op = op.multiply_operator(self._sig[j][b]).multiply_operator(
                        self._tau[j][c]
                    )
    ...
    def force_tau(
        ...
        for c in range(3):
            out += (
                self._tau[i][c]
                .multiply_operator(self._tau[j][c])
                .scale(coupling_array[i, j])
            )
        return out
    ...
    def force_coulomb(
        ...
        out = (
            ...
            + self._tau[i][2]
            + self._tau[j][2]
            + self._tau[i][2].multiply_operator(self._tau[j][2])
        )
    ...
    def propagator_spinorbit_linear(
        self, coupling_array: CouplingArray, i: int
    ) -> HilbertOperator:
        # linear approx to LS
        out = HilbertOperator(self.n_particles)
        for a in range(3):
            out = (
                self._ident - self._sig[i][a].scale(1.0j * coupling_array[a, i])
            ).multiply_operator(out)
        return out

    def propagator_spinorbit_onebody(
        self, coupling_array: CouplingArray, i: int
    ) -> HilbertOperator:
        # one-body part of the LS factorization
        out = HilbertOperator(self.n_particles).zero()
        for a in range(3):
            out += self._sig[i][a].scale(-1.0j * coupling_array[a, i])
        return out.exp()

    def propagator_spinorbit_twobody(
        self, coupling_array: CouplingArray, i: int, j: int
    ) -> HilbertOperator:
        ...
        for a in range(3):
            for b in range(3):
                out += (
                    self._sig[i][a]
                    .multiply_operator(self._sig[j][b])
                    .scale(0.5 * coupling_array[a, i] * coupling_array[b, j])
                )
        return out.exp()

    def force_sigma_3b(
        self, coupling_array: CouplingArray, i: int, j: int, k: int
    ) -> HilbertOperator:
        # 3-body sigma
        out = HilbertOperator(self.n_particles).zero()
        for a in range(3):
            for b in range(3):
                for c in range(3):
                    out += (
                        self._sig[i][a]
                        .multiply_operator(self._sig[j][b])
                        .multiply_operator(self._sig[k][c])
                        .scale(coupling_array[a, i, b, j, c, k])
                    )
        return out

    def propagator_combined(
        self,
        ...
        if spinorbit:
            if self._linear_spinorbit:
                for i in range(self.n_particles):
                    prop = self.propagator_spinorbit_linear(
                        potential.spinorbit, i
                    ).multiply_operator(prop)
            else:
                for i in range(self.n_particles):
                    prop = self.propagator_spinorbit_onebody(
                        potential.spinorbit, i
                    ).multiply_operator(prop)
                    for j in range(self.n_particles):
                        prop = self.propagator_spinorbit_twobody(
                            potential.spinorbit, i, j
                        ).multiply_operator(prop)
...
class HilbertPropagatorHS(Propagator):
    ...
    def __init__(
        self, n_particles: int, dt: float, isospin=True, include_prefactors=True
    ):
        ...
        self._sig_op = [
            [
                HilbertOperator(self.n_particles, isospin=isospin).apply_sigma(i, a)
                for a in [0, 1, 2, 3]
            ]
            for i in range(self.n_particles)
        ]
        self._tau_op = [
            [
                HilbertOperator(self.n_particles, isospin=isospin).apply_tau(i, a)
                for a in [0, 1, 2, 3]
            ]
            for i in range(self.n_particles)
        ]
    ...
    def twobody_sample(
        ...
        coupling: float,
        x: float,
        operator_i: HilbertOperator,
        operator_j: HilbertOperator,
    ) -> HilbertOperator:
    ...
    def factors_sigma(
        self, coupling_array: CouplingArray, aux: list
        ...
        for i, j in self._2b_idx:
            for a in self._xyz:
                for b in self._xyz:
                    if not coupling_array[a, i, b, j] == 0.0:
                        out.append(
                            self.twobody_sample(
                                coupling_array[a, i, b, j],
                                aux[idx],
                                self._sig_op[i][a],
                                self._sig_op[j][b],
                            )
        ...
        for i, j in self._2b_idx:
            for a in self._xyz:
                for b in self._xyz:
                    if not coupling_array[a, i, b, j] == 0.0:
                        for c in self._xyz:
                            opi = self._sig_op[i][a].multiply_operator(
                                self._tau_op[i][c]
                            )
                            opj = self._sig_op[j][b].multiply_operator(
                                self._tau_op[j][c]
                            )
                            out.append(
                                self.twobody_sample(
                                    coupling_array[a, i, b, j], aux[idx], opi, opj
                                )
                            )
                            idx += 1
        return out
        ...
        for i, j in self._2b_idx:
            if not coupling_array[i, j] == 0.0:
                if self.include_prefactors:
                    ...
                    out.append(
                        HilbertOperator(self.n_particles, self.isospin).scale(cexp(-z))
                    )
                out.append(
                    self.onebody(0.25 * coupling_array[i, j], self._tau_op[i][2])
                )
                out.append(
                    self.onebody(0.25 * coupling_array[i, j], self._tau_op[j][2])
                )
                out.append(
                    self.twobody_sample(
                        0.25 * coupling_array[i, j],
                        aux[idx],
                        self._tau_op[i][2],
                        self._tau_op[j][2],
                    )
    ...
    def factors_spinorbit(
        ...
        for i in self._1b_idx:
            for a in self._xyz:
                if not coupling_array[a, i] == 0.0:
                    z = 1.0j * coupling_array[a, i] / (self.dt * self.symmetry_factor)
                    out.append(self.onebody(z, self._sig_op[i][a]))
        for i, j in self._2b_idx:
            for a in self._xyz:
                for b in self._xyz:
                    if not coupling_array[a, i] == 0.0:
                        if not coupling_array[b, j] == 0.0:
                            z = (
                                -0.5
                                * coupling_array[a, i]
                                * coupling_array[b, j]
                                / (self.dt * self.symmetry_factor)
                            )
                            out.append(
                                self.twobody_sample(
                                    z, aux[idx], self._sig_op[i][a], self._sig_op[j][b]
                                )
                            )
                            idx += 1
        if self.include_prefactors:
...
class HilbertPropagatorRBM(Propagator):
    ...
    def threebody_sample(
        self,
        coupling: float,
        h_list: list,
        operator_i: HilbertOperator,
        operator_j: HilbertOperator,
        operator_k: HilbertOperator,
    ) -> HilbertOperator:
        ...
        out = (
            operator_i.scale(arg_i).exp()
            * operator_j.scale(arg_j).exp()
            * operator_k.scale(arg_k).exp()
        )
        return out.scale(prefactor)
    ...
    def factors_sigma_3b(
        ...
        for i, j, k in self._3b_idx:
            for a in self._xyz:
                for b in self._xyz:
                    for c in self._xyz:
                        if not coupling_array[a, i, b, j, c, k] == 0.0:
                            out.append(
                                self.threebody_sample(
                                    coupling_array[a, i, b, j, c, k],
                                    aux[idx : idx + 4],
                                    self._sig_op[i][a],
                                    self._sig_op[j][b],
                                    self._sig_op[k][c],
...
class ProductPropagatorHS(Propagator):
    ...
    def __init__(
        ...
        if isospin:
            ...
            self._sig = [
                kronecker_product([np.identity(2), pauli(a)]) for a in [0, 1, 2, 3]
            ]
            self._tau = [
                kronecker_product([pauli(a), np.identity(2)]) for a in [0, 1, 2, 3]
            ]
        else:
    ...
    def onebody(
        self, coupling: float, i: int, onebody_matrix: np.ndarray
    ) -> ProductOperator:
        r"""One-body propagator
        ...
        z = self.dt * self.symmetry_factor * coupling
        out.coefficients[i] = (
            ccosh(z) * out.coefficients[i]
            - csinh(z) * onebody_matrix @ out.coefficients[i]
        )
        return out
    ...
    def twobody_sample(
        ...
        i: int,
        j: int,
        onebody_matrix_i: np.ndarray,
        onebody_matrix_j: np.ndarray,
    ):
        r"""A sample of the two-body propagator in the integrand of the Hubbard-Stratonovich transform.
        ...
        out = ProductOperator(self.n_particles, self.isospin)
        out.coefficients[i] = (
            ccosh(arg) * out.coefficients[i]
            + csinh(arg) * onebody_matrix_i @ out.coefficients[i]
        )
        out.coefficients[j] = (
            ccosh(arg) * out.coefficients[j]
            + csinh(arg) * onebody_matrix_j @ out.coefficients[j]
        )
                        ...
                        for c in self._xyz:
                            out.append(
                                self.twobody_sample(
                                    ...
                                    i,
                                    j,
                                    self._sig[a] @ self._tau[c],
                                    self._sig[b] @ self._tau[c],
                                )
                            )
                            idx += 1
        return out
                ...
                for b in self._xyz:
                    if not coupling_array[a, i] == 0.0:
                        if not coupling_array[b, j] == 0.0:
                            ...
                            out.append(
                                self.twobody_sample(
                                    z, aux[idx], i, j, self._sig[a], self._sig[b]
                                )
                            )
                            idx += 1
        if self.include_prefactors:
            norm_op = ProductOperator(self.n_particles, self.isospin)
            norm_op = norm_op.scale_all(
                np.exp(0.5 * np.sum(coupling_array.coefficients**2))
            )
            out.append(norm_op)
        return out
...
class ProductPropagatorRBM(Propagator):
    ...
    def twobody_sample(
        ...
        out.coefficients[j] = (
            ccosh(arg) * out.coefficients[j]
            - S * csinh(arg) * onebody_matrix_j @ out.coefficients[j]
        )
        out.coefficients[i] *= csqrt(prefactor)
        out.coefficients[j] *= csqrt(prefactor)
        return out
    ...
    def threebody_sample(
        ...
        out.coefficients[i] = (
            ccosh(arg_i) * out.coefficients[i]
            + csinh(arg_i) * onebody_matrix_i @ out.coefficients[i]
        )
        out.coefficients[j] = (
            ccosh(arg_j) * out.coefficients[j]
            + csinh(arg_j) * onebody_matrix_j @ out.coefficients[j]
        )
        out.coefficients[k] = (
            ccosh(arg_k) * out.coefficients[k]
            + csinh(arg_k) * onebody_matrix_k @ out.coefficients[k]
        )
...
class Integrator:
    ...
    def setup(
        ...
        if self.method.lower() == "hs":
            ...
            if flip_aux:
                self.aux_fields_samples = (
                    np.ones_like(self.aux_fields_samples) - self.aux_fields_samples
                )
        self.is_ready = True
    ...
    def bracket(self, bra, ket, aux_fields):
        ...
        self.prop_list = []
        if self.sigma:
            self.prop_list.extend(
                self.propagator.factors_sigma(
                    self.potential.sigma,
                    aux_fields[idx : idx + self.propagator.n_aux_sigma],
                )
            )
            idx += self.propagator.n_aux_sigma
        if self.sigmatau:
            self.prop_list.extend(
                self.propagator.factors_sigmatau(
                    self.potential.sigmatau,
                    aux_fields[idx : idx + self.propagator.n_aux_sigmatau],
                )
            )
            idx += self.propagator.n_aux_sigmatau
        if self.tau:
            self.prop_list.extend(
                self.propagator.factors_tau(
                    self.potential.tau,
                    aux_fields[idx : idx + self.propagator.n_aux_tau],
                )
            )
            idx += self.propagator.n_aux_tau
        if self.coulomb:
            self.prop_list.extend(
                self.propagator.factors_coulomb(
                    self.potential.coulomb,
                    aux_fields[idx : idx + self.propagator.n_aux_coulomb],
                )
            )
            idx += self.propagator.n_aux_coulomb
        if self.spinorbit:
            self.prop_list.extend(
                self.propagator.factors_spinorbit(
                    self.potential.spinorbit,
                    aux_fields[idx : idx + self.propagator.n_aux_spinorbit],
                )
            )
            idx += self.propagator.n_aux_spinorbit
        if self.sigma_3b:
            self.prop_list.extend(
                self.propagator.factors_sigma_3b(
                    self.potential.sigma_3b,
                    aux_fields[idx : idx + self.propagator.n_aux_sigma_3b],
                )
            )
    ...
    def run(self, bra, ket):
        ...
        if self.parallel:
            with Pool(processes=self.n_processes) as pool:
                b_array = pool.starmap_async(
                    self.bracket,
                    tqdm(
                        [(bra, ket, aux) for aux in self.aux_fields_samples], leave=True
                    ),
                ).get()
        else:
            b_array = list(
                itertools.starmap(
                    self.bracket,
                    tqdm([(bra, ket, aux) for aux in self.aux_fields_samples]),
                )
            )
    ...
    def exact(self, bra, ket):
        ex = ExactPropagator(
            ...
            self.propagator.dt,
            isospin=self.isospin,
            include_prefactors=self.propagator.include_prefactors,
        )
        g_exact = ex.propagator_combined(
            self.potential,
            self.sigma,
            self.sigmatau,
            self.tau,
