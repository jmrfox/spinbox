...
def h_test(h):
    return [np.sum(h), np.mean(h), np.std(h)]
...
def gfmc_3b_1d(n_particles, dt, coupling, mode=1):
    ...
    sig = [
        [
            HilbertOperator(n_particles, isospin=isospin).apply_sigma(i, a)
            for a in [0, 1, 2]
        ]
    ...
    tau = [
        [
            HilbertOperator(n_particles, isospin=isospin).apply_tau(i, a)
            for a in [0, 1, 2]
        ]
        for i in range(n_particles)
    ]
    ...
    if mode == 1:  # rbm take 1: use 3b rbm and exact 2b
        ...
        for h in [0.0, 1.0]:
            ket_temp += (
                (ident.scale(-h * C) + (op_i + op_j + op_k).scale(A1 - h * W))
                .exp()
                .multiply_state(ket_prop)
            )
        ket_prop = ket_temp.scale(N)
        # 2b
        ket_prop = (
            (op_i * op_j + op_i * op_k + op_j * op_k)
            .scale(A2)
            .exp()
            .multiply_state(ket_prop)
        )
        ...
        print(
            "Using the 1-body part with RBM (sum over h) and a 2-body RBM for each of the 2-body parts (sum over h', h'', h''')"
        )
        ket_prop = ket.copy()  # outside loops
        ## 3b
        ket_temp = ket_prop.copy().zero()  # right before h loop
        ...
        for h in [0.0, 1.0]:
            ket_temp += (
                (ident.scale(-h * C3) + (op_i + op_j + op_k).scale((A1 - h * W3)))
                .exp()
                .multiply_state(ket_prop)
            )
        ...
        def boltz_f(h, a1, a2, w1, w2, c):
            s2 = a2 / cabs(a2)
            g1 = op_i.scale(
                w2 * (2 * h[1] - 1) + w2 * (2 * h[2] - 1) + a1 - h[0] * w1
            ).exp()
            g2 = op_j.scale(
                w2 * s2 * (2 * h[1] - 1) + w2 * (2 * h[3] - 1) + a1 - h[0] * w1
            ).exp()
            g3 = op_k.scale(
                w2 * s2 * (2 * h[2] - 1) + w2 * s2 * (2 * h[3] - 1) + a1 - h[0] * w1
            ).exp()
            out = g1 * g2 * g3
        ...
        print(
            "Using the 3-body RBM, sum over h, h', h'', h''', with the HilbertPropagatorRBM "
        )
        ...
        for h in h_list:
            b_array.append(
                bra * prop.threebody_sample(coupling, h, op_i, op_j, op_k) * ket
            )
        ...
        print(
            "RBM class, fake sampling 10K points, the aux field cycles through all 16 possible values (h, h', h'', h''')"
        )
        prop = HilbertPropagatorRBM(n_particles, dt, isospin)

        n_samples = 10000
        h_list = itertools.product([0, 1], repeat=4)
        h_list = (n_samples // 16) * list(
            h_list
        )  # list of 10K samples perfectly balanced
        ...
        for ih, h in enumerate(tqdm(h_list)):
            ...
            b_array[2 * ih + 1 : 2 * ih + 2] = (
                bra * prop.threebody_sample(coupling, 1 - h, op_i, op_j, op_k) * ket
            )
        b_rbm = np.mean(b_array)
        print(h_test(np.concatenate([h_list, 1 - h_list])))
    ...
    if mode == 3:
        ...
        def boltz_f(h, a1, a2, w1, w2, c):
            ...
            out.coefficients[i] = (
                ...
                + csinh(arg_i) * op_i @ out.coefficients[i]
            )
            out.coefficients[j] = (
                ccosh(arg_j) * out.coefficients[j]
                + csinh(arg_j) * op_j @ out.coefficients[j]
            )
            out.coefficients[k] = (
                ccosh(arg_k) * out.coefficients[k]
                + csinh(arg_k) * op_k @ out.coefficients[k]
            )
        ...
        b_array = []
        for h in h_list:
            ket_temp = prop.threebody_sample(
                dt * 0.5 * coupling, h, i, j, k, op_i, op_j, op_k
            ).multiply_state(ket.copy())
        ...
        for h in h_list:
            ...
            h_flip = 1 - h
            ket_temp = prop.threebody_sample(
                dt * 0.5 * coupling, h_flip, i, j, k, op_i, op_j, op_k
            ).multiply_state(ket.copy())
            b_array.append(bra * ket_temp)
        b_rbm = np.mean(b_array)
