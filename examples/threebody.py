...
def gfmc_3b_1d(n_particles, dt, a3):
    ...
    ket = AFDMCSpinIsospinState(n_particles, np.zeros(shape=(n_particles, 4, 1)), ketwise=True).randomize(0)
    bra = AFDMCSpinIsospinState(n_particles, np.zeros(shape=(n_particles, 1, 4)), ketwise=False).randomize(1)
...
def afdmc_3b_1d(n_particles, dt, a3):
    ...
    ##### rbm take 1: use 3b rbm and exact 2b
    prop_0 = AFDMCSpinIsospinOperator(n_particles)
    prop_1 = AFDMCSpinIsospinOperator(n_particles)
    
    # 3b
    N, C, W, A1, A2 = a3b_factors(0.5 * dt * a3)
    prop_0.op_stack[0] = (ccosh(A1)*ident + csinh(A1)*sig[0]) @ prop_0.op_stack[0]
    prop_0.op_stack[1] = (ccosh(A1)*ident + csinh(A1)*sig[0]) @ prop_0.op_stack[1]
    prop_0.op_stack[2] = (ccosh(A1)*ident + csinh(A1)*sig[0]) @ prop_0.op_stack[2]
    prop_1.op_stack[0] = cexp(-C/3)*(ccosh(A1-W)*ident + csinh(A1-W)*sig[0]) @ prop_1.op_stack[0]
    prop_1.op_stack[1] = cexp(-C/3)*(ccosh(A1-W)*ident + csinh(A1-W)*sig[0]) @ prop_1.op_stack[1]
    prop_1.op_stack[2] = cexp(-C/3)*(ccosh(A1-W)*ident + csinh(A1-W)*sig[0]) @ prop_1.op_stack[2]
    return N * ((bra * prop_0 * ket) + (bra * prop_1 * ket))
